...
import { categoryOptions, statusOptions, type PromiseItem, type Status } from "@/lib/types";
import { filterPromises } from "@/lib/utils";
import { t, statusLabel, type TranslationKey } from "@/lib/i18n";
...
const sortLabelKeys: Record<string, TranslationKey> = {
  id: "pe_sort_id",
  source_page: "pe_sort_page",
  importance: "pe_sort_importance",
  progress_desc: "pe_sort_progress_desc",
  progress_asc: "pe_sort_progress_asc",
};
...
export function PromiseExplorer({ promises }: { promises: PromiseItem[] }) {
  ...
  const startItem  = results.length ? (safePage - 1) * PAGE_SIZE + 1 : 0;
  const endItem    = Math.min(safePage * PAGE_SIZE, results.length);
  ...
  const activeChips = [
    query.trim() && { id: "query", label: `Search: ${query.trim()}` },
    collection !== "all" && { id: "collection", label: `View: ${t(lang, active.labelKey)}` },
    category !== "all" && { id: "category", label: category },
    status !== "all" && { id: "status", label: statusLabel(lang, status as Status) },
    importance !== "all" && {
      id: "importance",
      label: `Importance: ${t(lang, `imp_${importance}` as TranslationKey)}`,
    },
    sort !== "id" && {
      id: "sort",
      label: `Sort: ${t(lang, sortLabelKeys[sort] ?? "pe_sort_id")}`,
    },
  ...
  function clearChip(id: string) {
    ...
    if (id === "collection") setCollection("all");
    if (id === "category") setCategory("all");
    if (id === "status") setStatus("all");
    if (id === "importance") setImportance("all");
    if (id === "sort") setSort("id");
    setPage(1);
  }
  ...
  return (
    <div className="pe-wrapper" ref={topRef}>
      ...
      {activeChips.length > 0 && (
        <div className="pe-active-row" aria-label="Active filters">
          <span className={`pe-active-label${isMl ? " malayalam" : ""}`}>Refined by</span>
          {activeChips.map((chip) => (
            <button
              type="button"
              key={chip.id}
              className="pe-active-chip"
      ...
      <div className={`pe-info${isMl ? " malayalam" : ""}`}>
        <span className="pe-info-count">
          <strong>Showing {startItem.toLocaleString()}-{endItem.toLocaleString()}</strong>{" "}
          of <strong>{results.length.toLocaleString()}</strong>{" "}
